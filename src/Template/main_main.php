...
Необходимо реализовать API, следуя следующим требованиям:
    1. Создать в БД обязательную таблицу posts c обязательными полями id и content.
Допускается добавление любых других полей и таблиц, если это необходимо для решения задачи.
    2. Реализовать аутентификацию как метод API (модель выбирайте на своё усмотрение);
    3. Реализовать метод getPost, который позволит получать записи по id (если не передан id, тогда выбираются все записи) из таблицы posts.
    ...
    10.  Написать простейший интерфейс для использования метода setPost с применением Ajax (простая форма для записи данных в таблицу posts используя написанное API).
